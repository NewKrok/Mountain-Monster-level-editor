	...
	public class EditorWorld extends BaseUIComponent
	{
		...
		protected function drawLines( ):void
		{
			...
			if ( _nodeViews.length > 0 )
			{
				for ( var i:int = 0; i < _nodeViews.length; i++ )
				{
					...
					{
						_lineContainer.graphics.lineTo( snapToGrid( _nodeViews[i].x ), snapToGrid( _nodeViews[i].y ) );
					}
				}
				
				if ( _controlType == CONTROL_TYPE_ADD_NODE )
				{
					_lineContainer.graphics.lineTo( snapToGrid( mouseX ), snapToGrid( mouseY ) );
				}
			}
		}
		...
		protected function drawSelectedNodeMarker( ):void
		{
			...
			if ( _controlType == CONTROL_TYPE_SELECT && _nearestNode && _nearestNodeDistance < 30 )
			{
				...
				_graphicsMarkContainer.graphics.drawCircle( snapToGrid( _nearestNode.x ), snapToGrid( _nearestNode.y ), 10 );
				_graphicsMarkContainer.graphics.drawCircle( snapToGrid( _nearestNode.x ), snapToGrid( _nearestNode.y ), 7 );
			}
		}
		...
		protected function drawNewNodePositionMarker( ):void
		{
			...
			if ( _controlType == CONTROL_TYPE_ADD_NODE )
			{
				...
				_graphicsMarkContainer.graphics.drawCircle( snapToGrid( mouseX ), snapToGrid( mouseY ), 10 );
				_graphicsMarkContainer.graphics.drawCircle( snapToGrid( mouseX ), snapToGrid( mouseY ), 7 );
			}
		}
		...
		protected function onStageMouseUpHandlerForNode( e:MouseEvent ):void
		{
			if ( _draggedNode )
			{
				...
				_draggedNode.x = snapToGrid( _draggedNode.x );
				_draggedNode.y = snapToGrid( _draggedNode.y );
				drawLines( );
			}
		}
		...
		protected function addNodeTo( x:Number, y:Number ):void
		{
			...
			nodeView.x = snapToGrid( x );
			nodeView.y = snapToGrid( y );
